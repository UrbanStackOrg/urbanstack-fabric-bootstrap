...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"Org3MSP":.[1]}}}}}' config.json ./artifacts/org3.json > modified_config.json
set +x
...
setGlobals 0 2
set -x
peer channel update -f org3_update_in_envelope.pb -c ${CHANNEL_NAME} -o orderer.urbanstack.co:7050 --tls --cafile ${ORDERER_CA}
set +x
