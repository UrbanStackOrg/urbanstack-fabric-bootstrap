...
# and two Peer Orgs (``Aphp`` & ``Org2``) each managing and maintaining two peer nodes.
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for AphpMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./artifacts/AphpMSPanchors.tx -channelID $CHANNEL_NAME -asOrg AphpMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for AphpMSP..."
    exit 1
  fi
